...
const
  ...
  vertexShaderSource: PGLchar = '#version 330 core'
    ...
    + LE + 'out vec4 vertexColor; // 为片段着色器指定一个颜色输出'
    + LE + 'void main() '
    + LE + '{ '
    + LE + '  gl_Position = vec4(aPos, 1.0); // 注意我们如何把一个vec3作为vec4的构造器的参数'
    + LE + '  vertexColor = vec4(0.5, 0.0, 0.0, 1.0); // 把输出变量设置为暗红色'
    + LE + '} ';

  fragmentShaderSource: PGLchar = '#version 330 core'
    + LE + 'out vec4 FragColor;'
    + LE + 'in vec4 vertexColor; // 从顶点着色器传来的输入变量（名称相同、类型相同）'
    + LE + 'void main()'
    + LE + '{'
    + LE + '  FragColor = vertexColor;'
    + LE + '}';
  //═══════════════════════════════════════════════════════════════════════*)
  {$ENDREGION}
...
begin
  ...
  {═══════════════════════════════════════════════════════════════════════
  timevalue := GLfloat(0);
  greenValue := GLfloat(0);
  vertexColorLocation := GLint(0);
