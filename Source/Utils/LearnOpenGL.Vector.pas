...
type
  ...
  TglFloatArrayHelper = type Helper for TglFloatArray
    procedure AddglFloatf({%H-}f: GLfloat);
    procedure AddVector2f(const {%H-}Vertex: TVector2f);
    procedure AddVector3f(const {%H-}Vertex: TVector3f);
    procedure AddVector4f(const {%H-}Vertex: TVector4f);

    procedure AddFace2D(const {%H-}Face: TFace2D); overload;
    procedure AddFace2D(const v0, v1, v2: TVector2f); overload;
    procedure AddFace2DArray(const Face: array of TFace2D);

    procedure AddFace3D(const {%H-}Face: TFace3D); overload;
...
begin
  for i := 0 to 2 do
  begin
    if Self[i] < 0.0 then Self[i] := 0.0;
    if Self[i] > 1.0 then Self[i] := 1.0;
  end;
...
var
  i: integer;
begin
  for i := 0 to (Length(Self) - 1) div 2 do
  begin
    Self[i * 2 + 0] *= x;
    Self[i * 2 + 1] *= y;
  end;
...
var
  i: integer;
begin
  for i := 0 to (Length(Self) - 1) div 3 do
  begin
    Self[i * 3 + 0] *= x;
    Self[i * 3 + 1] *= y;
    Self[i * 3 + 2] *= z;
...
var
  ...
  v: TVector3f;
begin
  if Length(Normal) <> Length(Face) then Exit//    ShowMessage('Fehler: Lenght(Normal) <> Length(Face)');
  ;
